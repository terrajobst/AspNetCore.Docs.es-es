...
![Chismes](using-asynchronous-methods-in-aspnet-45/_static/image1.png)
...
Páginas asincrónicas de ASP.NET deben incluir el [página](https://msdn.microsoft.com/en-us/library/ydy4x04a.aspx) directiva con la `Async` atributo establecido en "true". El código siguiente muestra el [página](https://msdn.microsoft.com/en-us/library/ydy4x04a.aspx) directiva con la `Async` atributo establecido en "true" para el *GizmosAsync.aspx* página.
...
- El [página](https://msdn.microsoft.com/en-us/library/ydy4x04a.aspx) directiva debe tener la `Async` atributo establecido en "true".
...
- El nuevo `GetGizmosSvcAsync` método está marcado con el [async](https://msdn.microsoft.com/en-us/library/hh156513(VS.110).aspx) palabra clave, que indica al compilador para generar las devoluciones de llamada para las partes del cuerpo y para crear automáticamente un `Task` que se devuelve.
...
- Asincrónico [HttpClient](https://msdn.microsoft.com/en-us/library/system.net.http.httpclient(VS.110).aspx) clase se utiliza en lugar de sincrónico [WebClient](https://msdn.microsoft.com/en-us/library/system.net.webclient.aspx) clase.
- El [await](https://msdn.microsoft.com/en-us/library/hh156528(VS.110).aspx) palabra clave se aplica a la [HttpClient](https://msdn.microsoft.com/en-us/library/system.net.http.httpclient(VS.110).aspx)[GetAsync](https://msdn.microsoft.com/en-us/library/hh158944(VS.110).aspx) método asincrónico.
...
Métodos enlazan con `RegisterAsyncTask` se ejecutará inmediatamente después de [PreRender](https://msdn.microsoft.com/en-us/library/ms178472.aspx). También puede usar eventos en una página void async directamente, tal como se muestra en el código siguiente:
...
Los métodos asincrónicos que devuelven `Task`son cancelable, que es toman un [CancellationToken](https://msdn.microsoft.com/en-us/library/system.threading.cancellationtoken(VS.110).aspx) parámetro cuando se proporciona uno con el `AsyncTimeout` atributo de la [página](https://msdn.microsoft.com/en-us/library/ydy4x04a.aspx) directiva. El código siguiente muestra el *GizmosCancelAsync.aspx* página con un tiempo de espera de segundo.
...
- Tendrá que aumentar la [HTTP.sys](https://www.iis.net/learn/get-started/introduction-to-iis/introduction-to-iis-architecture) límite de la cola desde el valor predeterminado de 1.000 a 5000. Si el valor es demasiado bajo, es posible que vea [HTTP.sys](https://www.iis.net/learn/get-started/introduction-to-iis/introduction-to-iis-architecture) rechazar las solicitudes con el estado HTTP 503. Para cambiar el límite de la cola de HTTP.sys:
    ...
    - Haga clic con el botón secundario en el grupo de aplicaciones de destino y seleccione **configuración avanzada**.  
        ![avanzadas](using-asynchronous-methods-in-aspnet-45/_static/image4.png)
