...
ASP.NET 2.0 característica de asignación de sitio de s permite que un desarrollador de la página Definir un mapa del sitio web aplicación s en algún medio persistente, como en un archivo XML. Una vez definido, los datos del mapa del sitio pueden obtenerse acceso mediante programación a través del [ `SiteMap` clase](https://msdn.microsoft.com/en-us/library/system.web.sitemap.aspx) en el [ `System.Web` espacio de nombres](https://msdn.microsoft.com/en-us/library/system.web.aspx) o a través de una serie de exploración Web controles, como el Controles de SiteMapPath, Menu y vista de árbol. El sistema de asignación de sitio usa el [modelo de proveedor](http://aspnet.4guysfromrolla.com/articles/101905-1.aspx) para que se pueden crear implementaciones de serialización de mapa de sitio diferente y se ha conectado a una aplicación web. El proveedor de mapas de sitio predeterminado que se incluye con ASP.NET 2.0 conserva la estructura de mapa del sitio en un archivo XML. En el [páginas maestras y navegación del sitio](../introduction/master-pages-and-site-navigation-cs.md) tutorial hemos creado un archivo denominado `Web.sitemap` que contiene esta estructura y se ha actualizado su XML con cada nueva sección tutorial.
