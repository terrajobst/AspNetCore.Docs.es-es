...
* Una excepción que se produce en el método `ProductRepository.GetProductByIdAsync` se controla mediante una instrucción `try-catch`.
...
El marcado declarativo de un archivo de componente `.razor` se C# compila en un método denominado `BuildRenderTree`. Cuando se representa un componente, `BuildRenderTree` ejecuta y genera una estructura de datos que describe los elementos, el texto y los componentes secundarios del componente representado.
...
Para evitar una excepción de referencia nula en la lógica de representación, busque un objeto `null` antes de tener acceso a sus miembros. En el ejemplo siguiente, no se tiene acceso a las propiedades `person.Address` si `person.Address` es `null`:
...
Para obtener más información sobre la eliminación de componentes, vea <xref:blazor/components#component-disposal-with-idisposable>.
...
* Se rechaza el @no__t del lado de JavaScript-0.
...
Las notificaciones se administran registrando un servicio DI que hereda de la clase base abstracta `CircuitHandler`.
...
Cuando un circuito finaliza porque un usuario se ha desconectado y el marco está limpiando el estado del circuito, el marco de trabajo desecha el ámbito de DI del circuito. Al desechar el ámbito, se eliminan todos los servicios DI con ámbito de circuito que implementan <xref:System.IDisposable?displayProperty=fullName>. Si un servicio DI produce una excepción no controlada durante la eliminación, el marco de trabajo registra la excepción.

### <a name="prerendering"></a>Representación previa

Los componentes más increíbles se pueden representar con `Html.RenderComponentAsync`, de modo que su marcado HTML representado se devuelva como parte de la solicitud HTTP inicial del usuario. Esto funciona de la siguiente manera:
...
* Al tratar el circuito como `disconnected` hasta que el explorador del usuario establece una conexión Signalr en el mismo servidor para reanudar la interactividad en el circuito.
...
La mayoría de los componentes más increíbles se implementan como archivos *. Razor* y se compilan para generar lógica que opere en un `RenderTreeBuilder` para representar la salida. Un desarrollador puede implementar manualmente la lógica `RenderTreeBuilder` mediante C# código de procedimiento. Para obtener más información, consulta <xref:blazor/components#manual-rendertreebuilder-logic>.
...
Si se escribe el código `RenderTreeBuilder`, el desarrollador debe garantizar la corrección del código. Por ejemplo, el desarrollador debe asegurarse de que:
