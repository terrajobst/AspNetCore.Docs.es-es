...
El cambio más significativo de `WebHostBuilder` a `HostBuilder` en [inserción de dependencias (DI)](xref:fundamentals/dependency-injection). Cuando se usa `HostBuilder`, sólo puede insertar <xref:Microsoft.Extensions.Configuration.IConfiguration>, <xref:Microsoft.Extensions.Hosting.IHostingEnvironment>, y <xref:Microsoft.AspNetCore.Hosting.IHostingEnvironment> al constructor de inicio. El `HostBuilder` DI restricciones:
