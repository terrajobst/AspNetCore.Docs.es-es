...
* Establezca el [moniker de la plataforma de destino (TFM)](/dotnet/standard/frameworks) en `netcoreapp3.0`:
...
* Si la aplicación usa [analizadores de API](xref:web-api/advanced/analyzers), quite `<PackageReference>` cualquier elemento del paquete [Microsoft. AspNetCore. Mvc. API. analizadores](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.Api.Analyzers/) . Edite el archivo de proyecto para usar el analizador incluido como parte de la SDK de .NET Core:
...
* Si la aplicación usa escenarios de [CORS](xref:security/cors) , como `[EnableCors]`, coloque la llamada a `UseCors` antes de cualquier otro middleware que use CORS (por ejemplo, coloque `UseCors` antes `UseAuthentication`, `UseAuthorization`y `UseMvc`).
* Reemplace `IHostingEnvironment` `using` `Microsoft.Extensions.Hosting` por `IWebHostEnvironment` y agregue una instrucción para el espacio de nombres.
