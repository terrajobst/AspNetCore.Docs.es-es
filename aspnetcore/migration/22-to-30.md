...
El valor predeterminado para ASP.NET Core es ahora [System. Text. JSON](/dotnet/api/system.text.json?view=netcore-3.0), que es nuevo en .net Core 3,0. Considere la posibilidad de usar `System.Text.Json` cuando sea posible. Es de alto rendimiento y no requiere una dependencia de biblioteca adicional. Sin embargo, dado que `System.Text.Json` es nuevo, es posible que actualmente falten características que la aplicación necesita.
...
### <a name="systemruntimeserialization-attributes-not-supported"></a>Atributos de System. Runtime. Serialization no admitidos

Los atributos [[DataContract]](xref:System.Runtime.Serialization.DataContractAttribute) y [[DataMember]](xref:System.Runtime.Serialization.DataMemberAttribute) no se admiten en `System.Text.Json`. Para obtener más información, vea el tema sobre la compatibilidad de GitHub [System. Text. JSON con System. Runtime. Serialization](https://github.com/dotnet/corefx/issues/38758).
...
1. En el servidor, encadenar una llamada de método `AddNewtonsoftJsonProtocol` a la llamada al método `AddSignalR` en `Startup.ConfigureServices`:
