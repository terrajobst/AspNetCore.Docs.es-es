...
* MVC `Newtonsoft.Json` admite &ndash; compatibilidad con el uso de MVC con `Newtonsoft.Json` ahora forma parte de [Microsoft. AspNetCore. Mvc. NewtonsoftJson](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson).
...
La aplicación puede requerir `Newtonsoft.Json` integración si usa una característica específica de `Newtonsoft.Json` como JsonPatch o convertidores o si [da formato](xref:web-api/advanced/formatting) a tipos específicos de `Newtonsoft.Json`.

Para usar Json.NET en un proyecto de ASP.NET Core SignalR 3,0, consulte [cambiar a Newtonsoft. JSON](#switch-to-newtonsoftjson) en este documento.
...
Si la aplicación usa [controladores de autorización](xref:security/authorization/policies#authorization-handlers)personalizados, tenga en cuenta que el enrutamiento del punto de conexión pasa un tipo de recurso diferente a los controladores que MVC. Los controladores que esperan que el recurso de contexto del controlador de autorización sea del tipo <xref:Microsoft.AspNetCore.Mvc.Filters.AuthorizationFilterContext> (el tipo de recurso [proporcionado por los filtros de MVC](xref:security/authorization/policies#accessing-mvc-request-context-in-handlers)) deberán actualizarse para controlar los recursos de tipo <xref:Microsoft.AspNetCore.Routing.RouteEndpoint> (el tipo de recurso dado a los controladores de autorización por enrutamiento de extremo).
...
El cliente SignalR JavaScript ha cambiado de `@aspnet/signalr` a `@microsoft/signalr`. Para reaccionar a este cambio, cambie las referencias en los archivos *Package. JSON* , las instrucciones `require` y las instrucciones ECMAScript `import`.
...
1. Instale [Microsoft. AspNetCore.SignalR. Paquete NuGet Protocols. NewtonsoftJson](https://www.nuget.org/packages/Microsoft.AspNetCore.SignalR.Protocols.NewtonsoftJson) .
...
* [Algunos valores predeterminados de cookie SameSite cambian a ninguno (ASPNET/anuncios #348)](https://github.com/aspnet/Announcements/issues/348).
...
AddAuthorization se ha cambiado a otro ensamblado
