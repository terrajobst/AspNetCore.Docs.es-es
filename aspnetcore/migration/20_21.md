...
* Quite el atributo "Version" en la referencia de paquete a `Microsoft.AspNetCore.App`. Los proyectos que utilizan `<Project Sdk="Microsoft.NET.Sdk.Web">` no es necesario establecer la versión. La versión es implícito en la plataforma de destino y seleccionar para que mejor coincida con el funcionamiento de ASP.NET Core 2.1. Para obtener más información, consulte el [reglas para los proyectos destinados a marco compartido](#rules-for-projects-targeting-the-shared-framework) sección.
...
Problemas conocidos de los proyectos destinados a un marco compartido:
...
* Actualización de la  *\_LoginPartial.cshtml* archivo:
  ...
  * Actualización de la `<form />` elemento `<form asp-area="Identity" asp-page="/Account/Logout" asp-route-returnUrl="@Url.Page("/Index", new { area = "" })" method="post" id="logoutForm" class="navbar-right">`.
...
* En *áreas, Identity o páginas/\_ViewStart.cshtml*, cambiar `Layout = "/Pages/_Layout.cshtml"` a `Layout = "/Pages/Shared/_Layout.cshtml"`.
* El  *\_Layout.cshtml* archivo tiene los siguientes cambios:
