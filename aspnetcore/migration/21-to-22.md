...
Los proyectos que tienen como destino .NET Core deben usar el [TFM](/dotnet/standard/frameworks) de una versión mayor o igual que .net Core 2,2. En el archivo de proyecto, actualice `<TargetFramework>` el texto interno del nodo `netcoreapp2.2`con:
...
Para adoptar el [modelo de hospedaje en proceso para IIS](xref:host-and-deploy/iis/index#in-process-hosting-model), agregue la `<AspNetCoreHostingModel>` propiedad con un `<PropertyGroup>` valor de `InProcess` a en el archivo de proyecto:
...
* En la `<handlers>` entrada que agrega el módulo de ASP.net Core`name="aspNetCore"`(), cambie `modules` el valor del `AspNetCoreModule` atributo `AspNetCoreModuleV2`de a.
* En el `<aspNetCore>` elemento, agregue el atributo del modelo de`hostingModel="InProcess"`hospedaje ().

Para obtener más información y ejemplos de archivos *Web. config* de ejemplo, vea <xref:host-and-deploy/aspnet-core-module#configuration-with-webconfig>.

## <a name="update-package-references"></a>Actualización de las referencias del paquete

Si el destino es .net Core, quite el atributo de la `Version` referencia del metapaquete en el archivo de proyecto. La inclusión de `Version` un atributo produce la siguiente ADVERTENCIA:
...
La referencia del metapaquete debe ser similar al `<PackageReference />` siguiente nodo:
...
Si el destino es .NET Framework, actualice el atributo de `Version` cada referencia de paquete a 2.2.0 o posterior. Estas son las referencias de paquete en un proyecto de ASP.NET Core 2,2 típico que tiene como destino .NET Framework:
...
Si hace referencia al paquete [Microsoft. AspNetCore. Razor. Design](https://www.nuget.org/packages/Microsoft.AspNetCore.Razor.Design/) , actualice `Version` su atributo a 2.2.0 o posterior. Si no lo hace, se producirá el siguiente error:
...
Si la solución se basa en un archivo [global. JSON](/dotnet/core/tools/global-json) para tener como destino una versión de SDK de .net Core específica `version` , actualice su propiedad a la versión 2,2 instalada en el equipo:
...
Si usa Visual Studio Code, actualice el archivo de configuración de inicio del proyecto ( *. vscode/Launch. JSON*). La `program` ruta de acceso debe hacer referencia al nuevo TFM:
...
Si la aplicación no llama `CreateDefaultBuilder` a y compila el host manualmente en `Program` la clase, <xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*> llame a `ConfigureKestrel` **antes** de llamar a:
...
Actualice la versión de compatibilidad `Startup.ConfigureServices` en `Version_2_2`a:
...
Si la aplicación registra la inicialización del proveedor, el filtrado y la carga `Startup` de configuración en la clase, `Program.Main`mueva ese código a:
