...
Los proyectos destinados a .NET Core deben usar el [TFM](/dotnet/standard/frameworks) de una versión mayor o igual que .NET Core 2.2. En el archivo de proyecto, actualice el `<TargetFramework>` texto interno del nodo con `netcoreapp2.2`:
...
Adoptar la [en proceso modelo de hospedaje de IIS](xref:host-and-deploy/iis/index#in-process-hosting-model), agregue el `<AspNetCoreHostingModel>` propiedad con un valor de `InProcess` a un `<PropertyGroup>` en el archivo de proyecto:
...
Visual Studio **la generación automática de la solicitud del explorador** experiencia no funcionará con la [IIS modelo de hospedaje en proceso](xref:host-and-deploy/iis/index#in-process-hosting-model). Debe recompilar manualmente el proyecto cuando se usa en el proceso de hospedaje. Mejoras en esta experiencia se planean para una futura versión de Visual Studio.
