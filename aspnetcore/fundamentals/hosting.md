...
Impide la carga automática de los ensamblados de inicio de hospedaje, incluidos los configurados por el ensamblado de la aplicación. Para más información, vea [Mejorar una aplicación desde un ensamblado externo](xref:fundamentals/configuration/platform-specific-configuration).
...
[StopApplication](/dotnet/api/microsoft.aspnetcore.hosting.iapplicationlifetime.stopapplication) solicita la terminación de la aplicación. En la siguiente clase se usa `StopApplication` para cerrar correctamente una aplicación cuando se llama al método `Shutdown` de esa clase:
