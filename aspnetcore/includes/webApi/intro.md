...
* El cliente es todo lo que consume la API web (aplicación móvil, explorador, etcétera). En este tutorial no se crea ningún cliente. [Postman](https://www.getpostman.com/) o [curl](https://curl.haxx.se/docs/manpage.html) se utilizan como el cliente para probar la aplicación.

* Un *modelo* es un objeto que representa los datos de la aplicación. En este caso, el único modelo es una tarea pendiente. Los modelos se representan como clases de C#, también conocidas como clases POCO (del inglés **P**lain **O**ld **C**LR **O**bject, objetos CRL antiguos sin formato).
